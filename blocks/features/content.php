...
while (has_sub_field('features')) {
    ...
    if (is_array($link_link)) {
        $_features_content .= '<a class="field-link" target="' . $link_link['target'] . '" href="' . $link_link['url'] . '">' . $link_link['title'] . '</a>';
    }

    $_features_content .= '</li>';
}

?><div class="centered-container cc-block-features cc-block-features--<?php echo get_row_layout(); ?>">
